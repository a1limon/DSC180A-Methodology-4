...
class mcmc_log_reg:
    ...
    def log_posterior(self, y, X, beta, prior_means, prior_stds):
        ...
        Returns:
            [type]: [description]
        """
        prior = self.normal_log_prior(beta, prior_means, prior_stds)
        likelihood = self.log_likelihood(y, X, beta)
        return prior + likelihood
    
    def mh_mcmc(self, y, X, beta_priors, prior_stds,jumper_stds, num_steps,random_seed):
    ...
    def fit(self, method = 'median'):
        ...
        Args:
            method (str, optional): [description]. Defaults to 'median'.
        """
        if method == 'median':
